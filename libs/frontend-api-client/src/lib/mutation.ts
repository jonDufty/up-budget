...
export async function postMutation<T extends Record<string, any> | any[]>(
  endpoint: string,
  payload: T,
...
export function updateLocalData<T extends Record<string, any>>(
  existing: T[] | undefined,
  newData: T,
  key: string,
  options?: UpdateLocalOptions
) {
  if (!existing) {
    return existing;
  }
